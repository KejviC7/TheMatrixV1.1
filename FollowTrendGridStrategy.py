...
class TrendGridStrategy(): 
    ...
    def create_buy_orders(self):
        for i in range(self.config.NUM_BUY_GRID_LINES):
            ...
            try:
                order = exchange.create_order(symbol=self.config.SYMBOL, type = "limit", side = "buy", amount = self.config.CONTRACT_SIZE, price = price+1, params = {"marginMode": "isolated", "stopPrice": price, "timeInForce": "GTC"})
            except:
    ...
    def create_sell_orders(self):
        for i in range(self.config.NUM_SELL_GRID_LINES):
            ...
            order = exchange.create_order(symbol=self.config.SYMBOL, type = "limit", side = "sell", amount = self.config.CONTRACT_SIZE, price = price-1, params = {"marginMode": "isolated", "stopPrice": price, "timeInForce": "GTC"})
    ...
    def check_open_buy_orders(self):
        for buy_order in self.BUY_ORDERS:
            ...
            try:
                order = exchange.fetch_order(buy_order['id'], symbol = self.config.SYMBOL, params={'stop': True})
            except:
            ...
            if order['status'] == 'closed':
                ...
                #new_sell_price = new_ask_price + self.config.GRID_SIZE
                print(f" ************** Creating New Limit Sell Order at {new_sell_price} ***************** ")
                try:
                    new_sell_order = exchange.create_order(symbol=self.config.SYMBOL, type = "limit", side = "sell", amount = self.config.CONTRACT_SIZE, price = new_sell_price + 1, params = {"marginMode": "isolated", "stopPrice": new_sell_price, "timeInForce": "GTC"})
                except:
                    print(" \n<<<<<<<<<<<<<<<<  ERROR CREATING A TAKE PROFIT LIMIT SELL ORDER! >>>>>>>>>>>>>>>>>>>>> ")
    ...
    def check_open_sell_orders(self):
        for sell_order in self.SELL_ORDERS:
            ...
            try:
                order = exchange.fetch_order(sell_order['id'], symbol = self.config.SYMBOL, params={'stop': True})
            except:
            ...
            if order['status'] == 'closed':
                ...
                new_buy_price = new_bid_price - self.config.GRID_SIZE
                print(f" ************** Creating New Limit Buy Order at {new_buy_price} *************** ")
                try:
                    new_buy_order = exchange.create_order(symbol=self.config.SYMBOL, type = "limit", side = "buy", amount = self.config.CONTRACT_SIZE, price = new_buy_price-1, params = {"marginMode": "isolated", "stopPrice": new_buy_price, "timeInForce": "GTC"})
                except:
                    print(" \n<<<<<<<<<<<<<<<<  ERROR CREATING A TAKE PROFIT LIMIT BUY ORDER! >>>>>>>>>>>>>>>>>>>>> ")
    ...
    def close_all_positions(self):
        ...
        if pos_side == 'long':
            ...
            try:
                exchange.create_order(symbol=self.config.SYMBOL, type = "limit", side = 'long', amount = 0, price = 1.01 * bid, params = {"marginMode": "isolated", "close": True})
            except:
